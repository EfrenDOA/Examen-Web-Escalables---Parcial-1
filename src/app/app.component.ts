...
@Component({
  selector: 'app-root',
  standalone: true,
  imports: [RouterOutlet, NgFor, ShowCardComponent, FormsModule, NgIf, ReactiveFormsModule, ShowListComponent, NewShowFormComponent, FiltersComponent, EditInfoComponent],
...
export class AppComponent {
    ...
    public pushShow(newShow: Show): void {
        this.shows.push(newShow);
        this.showsCopy.push(newShow);
    }

    public showsCopy: Show[] = [...this.shows];

    public applyFilter(strFilter: string): void{
        // this.showsCopy.splice(0, this.showsCopy.length);
        // this.showsCopy = [...this.shows];
        if(strFilter == 'asc'){
            this.showsCopy.sort((a,b) => a.name.localeCompare(b.name));
        }else if(strFilter == 'desc'){
            this.showsCopy.sort((a,b) => b.name.localeCompare(a.name));
        }else if(strFilter == 'reverse'){
            this.showsCopy.reverse();
        }else if(strFilter == 'return'){
            this.showsCopy = [...this.shows];
        }
    }
    ...
    public inputShow(show: Show): void{
        this.showEdit = show;
        console.log("Sigue llegando", this.showEdit);
        this.id = this.shows.indexOf(this.showEdit);
    ...
    public changeValue(show:Show): void{
        console.log("new", this.newId);
        this.showsCopy[this.newId] = show;
    }
